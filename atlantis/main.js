...
function confirm_inv(id, pn, p, di, min, max, wallet)
{
	...
	$('#pack_inv').text(pn);
	$('#period').text(p);
	$('#intr').text(Math.fround(di*p).toFixed(2));
	$('#min').text(min);
	$('#max').text(max);
	$('#p_id').val(id);
...
$(document).ready(function(){
	$('#country').on('change', function(){		
        ...
        var mcountry = $('#mcountry');
		$('#p_loading').show();
		$.ajax
		({
		    url: "states.php?id="+data,
		    type: "get",
		    //dataType: 'json',
		    data: data,
		    success:function(result)
		    {
		        ...
		        {
		        	str = str + '<option value="'+dt[i]['name']+'">'+dt[i]['name']+'</option>';
		        }		                
		        $('#states').append(str);
...
{    
    ...
    var msg_entry = $('#msg_entry').val(); 
    $.ajax
    ({
        url: $('#comment_form').attr('action'),
        type: "post",                
        data: "tnx="+ticket_id+"&msg="+msg_entry,
        //dataType: "json",
       // cache: false,
        //contentType: false,
       // processData: false,
            ...
            else if(result.toast_type == 'suc' && user == 'user')
            {
                $('#chat_msg_container').append(
                    '<div class="row col-sm-12 d-flex justify-content-end">\
                        ...
                        <p class="mg_top_30">\
                           <img src="upload/'+result.user_img+'" alt="..." class="avatar_chat rounded-circle">\
                        </p>\
                    </div>'
                );
                $("#chat_msg_container").animate({ scrollTop: $('#chat_msg_container')[0].scrollHeight }, 1000); 
                        ...
                        '<div class="row col-sm-12 d-flex justify-content-end">\
                            ...
                            <div class="talk-bubble tri-right left-top ">\
                              <div class="talktext">\
                                ...
                                <p class="font_10 float-left"><i>'+result[i].date+'</i></p>\
                              </div>\
                            </div>\
                        </div>'
                    );
                            ...
                            '<div class="row col-sm-12">\
                                ...
                                <div class="talk-bubble tri-right left-top ">\
                                  <div class="talktext">\
                                    ...
                                    <small class="font_10 p-0 float-right"><i>'+result[i].date+'</i></small>\
                                  </div>\
                                </div>\
                            </div>'
                        );
...
$(document).ready( function(){
    $('#card_select').change(function(){
        if($('#card_select').val() == 'Country/State ID' || $('#card_select').val() == 'Drivers Licence')
        {
            $('#pass_cont').hide();
            $('#card_cont').show();
        }
        else if($('#card_select').val() == 'International Passport')
        {
            $('#card_cont').hide();
            $('#pass_cont').show();
        }
