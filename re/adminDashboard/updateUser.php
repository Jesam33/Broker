<?php
    ...
    if ($oldBalance > $newBalance) {
        ...
        $emailMessage->send_mail($email, $msg->debitUser($amount,$transaction_date,APP_NAME), "Account Notification");
        ...
        $balance = $newBalance;
        $emailMessage = new message(EMAIL_HOST, USER_NAME, PASSWORD, PORT, FROM, REPLY_TO);
        $emailMessage->send_mail($email, $msg->creditUser($amount,$transaction_date,APP_NAME), "Account Notification");
        $insertData->addTransHistory($email,$type,$amount,$balance,$status, $transaction_date);
        $insertData->addTransHistory($email,$type,$amount,$balance,$status, $transaction_date);
    }else{
    ...
    $headers = 'From: support@remitbrokers.com' . "\r\n";
    ...
    $message .= "</table>";
    $message .= "<p>If you did not initiate this transaction, please send us an email to support@remitbrokers.com</p>";
    $message .= "</body></html>";
    ...
    mb_send_mail($email, "Transactions from remitbrokers", $message,$headers);
